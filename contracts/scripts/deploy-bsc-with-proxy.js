...
async function main() {
  console.log('🚀 Deploying to BSC Testnet with Proxy...\n');
  ...
  // 4. Deploy UpgradeableMultiStakeManagerV2 as PROXY
  ...
  console.log('✅ UpgradeableMultiStakeManagerV2 PROXY deployed to:', await stakeManager.getAddress());
  console.log('✅ Implementation address:', await upgrades.erc1967.getImplementationAddress(await stakeManager.getAddress()));

  // 5. Set NFT Factory in StakeManager
  ...
  console.log('✅ StakeManager PROXY address:', await stakeManager.getAddress());
  ...
  // 9. Test version
  console.log('\n🔍 Testing contract version...');
  try {
    const version = await stakeManager.version();
    console.log('✅ Contract version:', version);
  } catch (error) {
    console.error('❌ Error getting version:', error.message);
  }

  console.log('\n🎉 BSC Testnet deployment with PROXY complete!');
  console.log('\n📋 Contract Addresses:');
  ...
  console.log('\n⚠️  IMPORTANT: Update your frontend config with these new addresses!');
}
