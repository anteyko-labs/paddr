...
async function main() {
    console.log("🚀 Upgrading PADToken to V3 with Blacklist functionality...");
    ...
    console.log("🔨 Deploying PADTokenV3...");
    const PADTokenV3 = await ethers.getContractFactory("PADTokenV3");
    
    // Upgrade the proxy (same address!)
    console.log("⬆️ Upgrading token proxy to V3...");
    const upgraded = await upgrades.upgradeProxy(PROXY_ADDRESS, PADTokenV3);
    ...
    // Test new functions
    console.log("🧪 Testing new V3 functions...");
    ...
    console.log("\n🎉 Token upgrade to V3 completed successfully!");
...
main()
    .then(() => process.exit(0))
    .catch((error) => {
        console.error("❌ Error during token V3 upgrade:", error);
        process.exit(1);
    });
