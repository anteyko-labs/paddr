...
import { Trophy, Video, ExternalLink, Copy } from 'lucide-react';
...
export function NFTGallery() {
      ...
      try {
        ...
        for (let i = 0; i < Number(balance); i++) {
          try {
            ...
            if (tokenId) {
              // Получаем URI токена
              const tokenURI = await fetch(`/api/nft-token-uri?tokenId=${tokenId}`)
                .then(res => res.json())
                .then(data => data.tokenURI);

              // Получаем метаданные NFT
              const metadata = await fetch(`/api/nft-metadata?tokenId=${tokenId}`)
                .then(res => res.json())
                .then(data => data);
              ...
              // Используем видео из метаданных
  ...
  const openInWallet = (tokenId: string) => {
    // Открываем в блокчейн эксплорере
                ...
                <div className="aspect-video bg-gray-100 relative">
                  {nft.videoUrl ? (
                    <video
                      ...
                      controls
                      poster={`/api/nft-thumbnail?tokenId=${nft.tokenId}`}
                    >
